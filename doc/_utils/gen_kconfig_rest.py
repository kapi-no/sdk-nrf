...
def main():
    init() # updated
    kconf.unique_defined_syms = [sym for sym in kconf.unique_defined_syms if sym.name.startswith("FMNA") or sym.name.startswith("UARP")]
...
def choice_id(choice):
    ...
    index = choice.kconfig.unique_choices.index(choice)
    used_choices[id(choice.kconfig.unique_choices[index])] = True

...
LOG_MODULE_REGISTER(fp_fhn_pf, CONFIG_BT_FAST_PAIR_LOG_LEVEL);
...
bool pf_ranging_req_handle(struct ranging_comm_chan *chan,
			   struct fp_fhn_pf_oob_msg *req,
			   struct net_buf_simple *req_buf,
			   enum fp_fhn_pf_oob_msg_id msg_id)
{
	int err;
	...
	err = fp_fhn_pf_oob_msg_decode(req_buf, req);
	if (err) {
	...
	if (req->version > OOB_MSG_VERSION) {
		if (msg_id != FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_REQ) {
			LOG_WRN("Precision Finding: Ranging Request (0x%02X): "
...
bool fp_fhn_pf_ranging_capability_req_handle(struct bt_conn *conn,
					     struct net_buf_simple *req_buf)
{
	struct ranging_comm_chan *chan;
	struct fp_fhn_pf_oob_msg req = {0};
	enum fp_fhn_pf_oob_msg_id msg_id = FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_REQ;
...
bool fp_fhn_pf_ranging_config_req_handle(struct bt_conn *conn,
					 struct net_buf_simple *req_buf)
{
	struct ranging_comm_chan *chan;
	struct bt_fast_pair_fhn_pf_ranging_tech_payload config_payloads[
		FP_FHN_PF_OOB_MSG_RANGING_TECH_PAYLOAD_MAX_NUM];
	struct fp_fhn_pf_oob_msg req = {0};
	enum fp_fhn_pf_oob_msg_id msg_id = FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_REQ;
...
bool fp_fhn_pf_stop_ranging_req_handle(struct bt_conn *conn,
				       struct net_buf_simple *req_buf)
{
	struct ranging_comm_chan *chan;
	struct fp_fhn_pf_oob_msg req = {0};
	enum fp_fhn_pf_oob_msg_id msg_id = FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_REQ;
				  ...
				  struct net_buf_simple *rsp_buf)
{
	...
	err = fp_fhn_pf_oob_msg_encode(rsp, rsp_buf);
	if (err) {
...
int fp_fhn_pf_ranging_capability_rsp_prepare(
	struct bt_conn *conn,
	uint16_t ranging_tech_bm,
...
{
	struct fp_fhn_pf_oob_msg rsp = {
		.id = FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_RSP,
...
int fp_fhn_pf_ranging_config_rsp_prepare(struct bt_conn *conn,
					 uint16_t ranging_tech_bm,
					 struct net_buf_simple *rsp_buf)
{
	struct fp_fhn_pf_oob_msg rsp = {
		.id = FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_RSP,
		.version = OOB_MSG_VERSION,
		.ranging_config_rsp = {
			.ranging_tech_bm = ranging_tech_bm,
		},
...
int fp_fhn_pf_stop_ranging_rsp_prepare(struct bt_conn *conn,
				       uint16_t ranging_tech_bm,
				       struct net_buf_simple *rsp_buf)
{
	struct fp_fhn_pf_oob_msg rsp = {
		.id = FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_RSP,
		.version = OOB_MSG_VERSION,
		.stop_ranging_rsp = {
			.ranging_tech_bm = ranging_tech_bm,
		},
...
void fp_fhn_pf_ranging_rsp_sent_notify(struct bt_conn *conn, uint16_t ranging_tech_bm)
{
...
void fp_fhn_pf_account_key_set(struct bt_conn *conn, struct fp_account_key *account_key)
{
	struct ranging_comm_chan *chan;
...
struct fp_account_key *fp_fhn_pf_account_key_get(struct bt_conn *conn)
{
	struct ranging_comm_chan *chan;
...
static void pf_oob_msg_len_validate(void)
{
	...
	max_len = fp_fhn_pf_oob_msg_max_len_get(FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_RSP);
	__ASSERT_NO_MSG(max_len == FP_FHN_PF_RANGING_CAPABILITY_RSP_MAX_LEN);

	max_len = fp_fhn_pf_oob_msg_max_len_get(FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_RSP);
	__ASSERT_NO_MSG(max_len == FP_FHN_PF_RANGING_CONFIG_RSP_LEN);

	max_len = fp_fhn_pf_oob_msg_max_len_get(FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_RSP);
	__ASSERT_NO_MSG(max_len == FP_FHN_PF_STOP_RANGING_RSP_LEN);
}
...
FP_ACTIVATION_MODULE_REGISTER(fp_fhn_pf,

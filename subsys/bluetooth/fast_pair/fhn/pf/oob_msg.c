...
LOG_MODULE_REGISTER(fp_fhn_pf_oob_msg, CONFIG_BT_FAST_PAIR_LOG_LEVEL);
 ...
 */
BUILD_ASSERT(FP_FHN_PF_OOB_MSG_RANGING_TECH_PAYLOAD_MAX_NUM ==
...
static int pf_oob_msg_ranging_capability_req_decode(struct net_buf_simple *msg_buf,
						    struct fp_fhn_pf_oob_msg *msg_desc)
{
	__ASSERT_NO_MSG(msg_desc->id == FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_REQ);
						...
						struct fp_fhn_pf_oob_msg *msg_desc)
{
	...
	__ASSERT_NO_MSG(msg_desc->id == FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_REQ);
	...
	__ASSERT_NO_MSG(config_payload_num <= FP_FHN_PF_OOB_MSG_RANGING_TECH_PAYLOAD_MAX_NUM);
...
static int pf_oob_msg_stop_ranging_req_decode(struct net_buf_simple *msg_buf,
					      struct fp_fhn_pf_oob_msg *msg_desc)
{
	__ASSERT_NO_MSG(msg_desc->id == FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_REQ);
...
int fp_fhn_pf_oob_msg_decode(struct net_buf_simple *msg_buf,
			     struct fp_fhn_pf_oob_msg *msg_desc)
{
	...
	switch (msg_desc->id) {
	case FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_REQ:
		return pf_oob_msg_ranging_capability_req_decode(msg_buf, msg_desc);
	case FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_REQ:
		return pf_oob_msg_ranging_config_req_decode(msg_buf, msg_desc);
	case FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_REQ:
...
static uint8_t pf_oob_msg_len_get(const struct fp_fhn_pf_oob_msg *msg_desc)
{
	struct bt_fast_pair_fhn_pf_ranging_tech_payload *payload;
...
static int pf_oob_msg_ranging_capability_rsp_encode(const struct fp_fhn_pf_oob_msg *msg_desc,
						    struct net_buf_simple *msg_buf)
{
	...
	__ASSERT_NO_MSG(msg_desc->id == FP_FHN_PF_OOB_MSG_ID_RANGING_CAPABILITY_RSP);
...
static int pf_oob_msg_ranging_config_rsp_encode(const struct fp_fhn_pf_oob_msg *msg_desc,
						struct net_buf_simple *msg_buf)
{
	uint8_t msg_buf_min_len;

	__ASSERT_NO_MSG(msg_desc->id == FP_FHN_PF_OOB_MSG_ID_RANGING_CONFIG_RSP);
...
static int pf_oob_msg_stop_ranging_rsp_encode(const struct fp_fhn_pf_oob_msg *msg_desc,
					      struct net_buf_simple *msg_buf)
{
	uint8_t msg_buf_min_len;

	__ASSERT_NO_MSG(msg_desc->id == FP_FHN_PF_OOB_MSG_ID_STOP_RANGING_RSP);
...
int fp_fhn_pf_oob_msg_encode(const struct fp_fhn_pf_oob_msg *msg_desc,
			     struct net_buf_simple *msg_buf)
{
	__ASSERT_NO_MSG(msg_desc);
	__ASSERT_NO_MSG(msg_buf);
...
uint8_t fp_fhn_pf_oob_msg_max_len_get(enum fp_fhn_pf_oob_msg_id msg_id)
{
	uint8_t len = OOB_MSG_MSG_ID_HDR_LEN;

					   ...
					   struct net_buf_simple *req_data_buf)
{
	...
	if (!IS_ENABLED(CONFIG_BT_FAST_PAIR_FHN_PF)) {
		LOG_WRN("Beacon Actions: unhandled request for Precision Finding "
			"(CONFIG_BT_FAST_PAIR_FHN_PF=n): data_id=%d", pf_data_id);
		return BT_GATT_ERR(BEACON_ACTIONS_ATT_ERR_INVALID_VALUE);
	}
	...
	fp_fhn_pf_account_key_set(conn, &account_key);

	switch (pf_data_id) {
	case BEACON_ACTIONS_PF_RANGING_CAPABILITY_REQ:
		ret = fp_fhn_pf_ranging_capability_req_handle(conn, req_data_buf);
		break;
	case BEACON_ACTIONS_PF_RANGING_CONFIG_REQ:
		ret = fp_fhn_pf_ranging_config_req_handle(conn, req_data_buf);
		break;
	case BEACON_ACTIONS_PF_STOP_RANGING_REQ:
		ret = fp_fhn_pf_stop_ranging_req_handle(conn, req_data_buf);
		break;
	default:
				    ...
				    struct net_buf_simple *ranging_oob_msg_buf)
{
	...
	account_key = fp_fhn_pf_account_key_get(conn);
	if (!account_key) {
		LOG_ERR("Beacon Actions: Ranging Response message (0x%02X):"
			" fp_fhn_pf_account_key_get failed: %d", data_id, err);
	...
	if (err) {
		LOG_ERR("Beacon Actions: Ranging Response message (0x%02X):"
			" fp_fhn_auth_seg_generate failed: %d", data_id, err);
	...
	fp_fhn_pf_ranging_rsp_sent_notify(conn, ranging_tech_bm);
	...
	uint8_t capability_payload_num)
{
	...
	NET_BUF_SIMPLE_DEFINE(ranging_oob_msg_buf, FP_FHN_PF_RANGING_CAPABILITY_RSP_MAX_LEN);
	NET_BUF_SIMPLE_DEFINE(rsp_buf,
			      (BEACON_ACTIONS_HEADER_LEN + BEACON_ACTIONS_RSP_AUTH_SEG_LEN +
			       FP_FHN_PF_RANGING_CAPABILITY_RSP_MAX_LEN));

	err = fp_fhn_pf_ranging_capability_rsp_prepare(conn,
						       ranging_tech_bm,
						       capability_payloads,
	...
	if (err) {
		LOG_ERR("Beacon Actions: Ranging Capability Response message:"
			" fp_fhn_pf_ranging_capability_rsp_prepare failed: %d", err);
						     ...
						     uint16_t ranging_tech_bm)
{
	...
	NET_BUF_SIMPLE_DEFINE(ranging_oob_msg_buf, FP_FHN_PF_RANGING_CONFIG_RSP_LEN);
	NET_BUF_SIMPLE_DEFINE(rsp_buf,
			      (BEACON_ACTIONS_HEADER_LEN + BEACON_ACTIONS_RSP_AUTH_SEG_LEN +
			       FP_FHN_PF_RANGING_CONFIG_RSP_LEN));

	err = fp_fhn_pf_ranging_config_rsp_prepare(conn, ranging_tech_bm, &ranging_oob_msg_buf);
	if (err) {
		LOG_ERR("Beacon Actions: Ranging Configuration Response message:"
			" fp_fhn_pf_ranging_config_rsp_prepare failed: %d", err);
						   ...
						   uint16_t ranging_tech_bm)
{
	...
	NET_BUF_SIMPLE_DEFINE(ranging_oob_msg_buf, FP_FHN_PF_STOP_RANGING_RSP_LEN);
	NET_BUF_SIMPLE_DEFINE(rsp_buf,
			      (BEACON_ACTIONS_HEADER_LEN + BEACON_ACTIONS_RSP_AUTH_SEG_LEN +
			       FP_FHN_PF_STOP_RANGING_RSP_LEN));

	err = fp_fhn_pf_stop_ranging_rsp_prepare(conn, ranging_tech_bm, &ranging_oob_msg_buf);
	if (err) {
		LOG_ERR("Beacon Actions: Stop Ranging Response message:"
			" fp_fhn_pf_stop_ranging_rsp_prepare failed: %d", err);
				    ...
				    uint8_t flags)
{
	...
	case BEACON_ACTIONS_PF_RANGING_CONFIG_REQ:
	case BEACON_ACTIONS_PF_STOP_RANGING_REQ:
		res = pf_ranging_operation_handle(data_id, conn, attr, &fhn_beacon_actions_buf);
		break;
	default:

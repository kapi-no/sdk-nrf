...
LOG_MODULE_REGISTER(fp_fhn_state, CONFIG_BT_FAST_PAIR_LOG_LEVEL);
...
#define FHN_FRAME_TYPE_OFFSET         (0 + FHN_FRAME_UUID_LEN)
#define FHN_FRAME_TYPE_LEN            1
...
#define FHN_FRAME_HASHED_FLAGS_UTP_BIT_OFFSET     0
#define FHN_FRAME_HASHED_FLAGS_UTP_BIT_LEN        1
#define FHN_FRAME_HASHED_FLAGS_BATTERY_BIT_OFFSET 1
#define FHN_FRAME_HASHED_FLAGS_BATTERY_BIT_LEN    2

/* Constants used in the FHN advertising frame. */
#define FHN_FRAME_UUID 0xFEAA
...
#define FHN_EID_SEED_PADDING_TYPE_ONE 0xFF
#define FHN_EID_SEED_PADDING_TYPE_TWO 0x00

/* Limits in seconds to the EID rotation period randomness as recommended by the specification. */
#define FHN_EID_ROT_PERIOD_RAND_LOWER_LIMIT 1
#define FHN_EID_ROT_PERIOD_RAND_UPPER_LIMIT 204
#define FHN_EID_ROT_PERIOD_RAND_LIMIT_DIFF     \
	(FHN_EID_ROT_PERIOD_RAND_UPPER_LIMIT - \
	FHN_EID_ROT_PERIOD_RAND_LOWER_LIMIT)

/* Maximum number of FHN connection slots. */
#define FHN_MAX_CONN (CONFIG_BT_FAST_PAIR_FHN_MAX_CONN)

/* FHN TX power settings. */
#define FHN_TX_POWER                (CONFIG_BT_FAST_PAIR_FHN_TX_POWER)
#define FHN_TX_POWER_CORRECTION_VAL (CONFIG_BT_FAST_PAIR_FHN_TX_POWER_CORRECTION_VAL)
#define FHN_TX_POWER_CALIBRATED_MIN (-100)
#define FHN_TX_POWER_CALIBRATED_MAX (20)
...
/* Reserve at least two connection slots for FHN connections and advertising. */
BUILD_ASSERT(CONFIG_BT_MAX_CONN > FHN_MAX_CONN);

/* Validate the Elliptic Curve configuration. */
BUILD_ASSERT(IS_ENABLED(CONFIG_BT_FAST_PAIR_FHN_ECC_SECP256R1) ||
	     IS_ENABLED(CONFIG_BT_FAST_PAIR_FHN_ECC_SECP160R1));
BUILD_ASSERT((SECP_MOD_RES_LEN == FP_CRYPTO_ECC_SECP160R1_MOD_LEN) ||
	     (SECP_MOD_RES_LEN == FP_CRYPTO_ECC_SECP256R1_MOD_LEN));

static uint8_t fhn_frame_payload[FHN_FRAME_PAYLOAD_LEN] = {
	BT_UUID_16_ENCODE(FHN_FRAME_UUID), FHN_FRAME_TYPE_UTP_MODE_OFF,
};

static const struct bt_data fhn_frame_data[] = {
	BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
	BT_DATA(BT_DATA_SVC_DATA16, fhn_frame_payload, sizeof(fhn_frame_payload)),
};

static struct bt_le_ext_adv *fhn_adv_set;
static struct bt_le_adv_param fhn_adv_param = {
	.id = BT_ID_DEFAULT,
	.options =
		/* Enable Extended Advertising for the SECP256R1 ECC variant. */
		(IS_ENABLED(CONFIG_BT_FAST_PAIR_FHN_ECC_SECP256R1) * BT_LE_ADV_OPT_EXT_ADV) |
		BT_LE_ADV_OPT_CONN,
};
static int8_t fhn_adv_set_tx_power;

static uint8_t * const fhn_frame_type = (fhn_frame_payload + FHN_FRAME_TYPE_OFFSET);
static uint8_t * const fhn_frame_hashed_flags =
	(fhn_frame_payload + FHN_FRAME_HASHED_FLAGS_OFFSET);
static uint8_t fhn_frame_hashed_flags_xor_operand;

static uint8_t * const fhn_eid = (fhn_frame_payload + FHN_FRAME_EID_OFFSET);
static uint32_t fhn_eid_clock_checkpoint;
...
static uint32_t fhn_conn_cnt;
static bool fhn_conns[CONFIG_BT_MAX_CONN];

static void fhn_disconnected_work_handle(struct k_work *work);
static void fhn_post_init_work_handle(struct k_work *work);

static K_WORK_DEFINE(fhn_disconnected_work, fhn_disconnected_work_handle);
static K_WORK_DEFINE(fhn_post_init_work, fhn_post_init_work_handle);

static int fhn_adv_start(void);
				 ...
				 uint32_t fhn_clock)
{
	...
	net_buf_simple_add_u8(buf, FHN_EID_SEED_ROT_PERIOD_EXP);
	net_buf_simple_add_be32(buf, fhn_clock);
}
...
static int eid_encode(void)
{
	...
	if (memcmp(fhn_eid, uninitialized_eid, sizeof(uninitialized_eid)) != 0) {
		if (fhn_clock == fhn_eid_clock_checkpoint) {
			LOG_DBG("FHN State: EID does not require recalculation");
	...
	eid_seed_half_encode(&eid_seed_buf,
			     FHN_EID_SEED_PADDING_TYPE_ONE,
			     fhn_clock);
	eid_seed_half_encode(&eid_seed_buf,
			     FHN_EID_SEED_PADDING_TYPE_TWO,
			     fhn_clock);
...
static uint8_t frame_type_encode(void)
{
	...
	frame_type_byte = utp_mode ?
		FHN_FRAME_TYPE_UTP_MODE_ON :
		FHN_FRAME_TYPE_UTP_MODE_OFF;

	LOG_DBG("FHN State: FHN Frame type: 0x%02X", frame_type_byte);
...
static uint8_t hashed_flags_encode(void)
{
	...
	enum fp_fhn_battery_level battery_level;

	hashed_flags_seed = 0;
	battery_level = fp_fhn_battery_level_get();
	WRITE_BIT(hashed_flags_seed, FHN_FRAME_HASHED_FLAGS_UTP_BIT_OFFSET, utp_mode);
	hashed_flags_seed |= (battery_level << FHN_FRAME_HASHED_FLAGS_BATTERY_BIT_OFFSET);

	hashed_flags_byte = (hashed_flags_seed ^ fhn_frame_hashed_flags_xor_operand);

	LOG_DBG("FHN State: Hashed Flags byte:");
	LOG_DBG("\tBitmask:\t0x%02X", hashed_flags_seed);
	LOG_DBG("\tHash:\t\t0x%02X", fhn_frame_hashed_flags_xor_operand);
	LOG_DBG("\tFinal value:\t0x%02X", hashed_flags_byte);

	return hashed_flags_byte;
}

static int fhn_adv_data_async_update(void)
{
	int err;

	if (!fhn_adv_set) {
		/* FHN advertising set is not active. */
		return 0;
	}

	/* Verify that the frame type does not change during async update. */
	__ASSERT(*fhn_frame_type == frame_type_encode(),
		"FHN State: frame type should not change during async update");

	/* Prepare the Hashed Flags byte in the new advertising payload. */
	*fhn_frame_hashed_flags = hashed_flags_encode();
	...
	LOG_DBG("FHN State: updating the Hashed Flags byte in advertising payload");

	return 0;
}

static int fhn_adv_payload_generate(void)
{
	int err;

	err = eid_encode();
	if (err) {
		LOG_ERR("FHN State: eid_encode returned error: %d", err);
		return err;
	}
	...
	*fhn_frame_hashed_flags = hashed_flags_encode();
	...
	/* Set the FHN frame type with the updated UTP Mode indication. */
	*fhn_frame_type = frame_type_encode();

	return 0;
}

static uint16_t fhn_adv_rpa_timeout_calculate(void)
{
	int err;
	...
	fhn_clock = fp_fhn_clock_read();
	non_rand_rotation_time = BIT(FHN_EID_SEED_ROT_PERIOD_EXP);
	non_rand_rotation_time -= fhn_clock % BIT(FHN_EID_SEED_ROT_PERIOD_EXP);

	/* Calculate the positive randomized time factor. */
	err = sys_csrand_get(&rand_rotation_time_seed, sizeof(rand_rotation_time_seed));
	if (err) {
		LOG_WRN("FHN State: sys_csrand_get failed: %d", err);

		sys_rand_get(&rand_rotation_time_seed, sizeof(rand_rotation_time_seed));
	}

	BUILD_ASSERT(FHN_EID_ROT_PERIOD_RAND_LIMIT_DIFF < UINT8_MAX);

	/* Convert the random part range from <0; UINT8_MAX> to
	 * <FHN_EID_ROT_PERIOD_RAND_LOWER_LIMIT;
	 *  FHN_EID_ROT_PERIOD_RAND_UPPER_LIMIT>
	 */
	rand_rotation_time = FHN_EID_ROT_PERIOD_RAND_LIMIT_DIFF;
	rand_rotation_time *= rand_rotation_time_seed;
	rand_rotation_time /= UINT8_MAX;
	rand_rotation_time += FHN_EID_ROT_PERIOD_RAND_LOWER_LIMIT;

	__ASSERT((non_rand_rotation_time + rand_rotation_time) < UINT16_MAX,
		"FHN State: Invalid RPA timeout calculation");

	LOG_DBG("FHN State: calculating new rotation time: %d s + %d s",
		non_rand_rotation_time, rand_rotation_time);

	return (non_rand_rotation_time + rand_rotation_time);
}

static bool fhn_adv_rpa_expired_state_transition(void)
{
	bool rotate_rpa;

	if (fhn_conn_cnt == FHN_MAX_CONN) {
		LOG_DBG("FHN State: RPA locked on disabled advertising");
		 ...
		 * sets in the FHN provisioned state even if the FHN
	...
	if (utp_mode) {
		...
		if (utp_rotations == UTP_EID_ROTATIONS_PER_RPA_ROTATION) {
			...
			LOG_DBG("FHN State: UTP mode: executing daily RPA rotation");
		} else {
			rotate_rpa = false;

			LOG_DBG("FHN State: UTP mode: RPA locked for this EID rotation");
		}
	}
...
static bool fhn_adv_rpa_expired(struct bt_le_ext_adv *adv)
{
	int err;
	bool rotate_rpa;
	...
	BUILD_ASSERT(!IS_ENABLED(CONFIG_BT_RPA_SHARING),
		     "RPA sharing is not supported when combined with the FHN extension");
	...
	if (!bt_fast_pair_fhn_is_provisioned()) {
		 ...
		 * received on a forced RPA rotation during the FHN provisioning operation (and
		 * just before the start of the FHN advertising). The RPA expired callback will
	...
	LOG_DBG("FHN State: RPA expired");

	/* Handle state changes related to the RPA rotation. */
	rotate_rpa = fhn_adv_rpa_expired_state_transition();

	/* Randomize the next rotation period on the RPA timeout. */
	next_rpa_timeout = fhn_adv_rpa_timeout_calculate();
	err = bt_le_set_rpa_timeout(next_rpa_timeout);
	if (err) {
		LOG_ERR("FHN State: bt_set_rpa_timeout failed: %d", err);
	}

	/* Prepare the advertising payload for the FHN frame. */
	err = fhn_adv_payload_generate();
	if (err) {
		LOG_ERR("FHN State: generation of advertising payload failed: %d", err);
	}
	...
	LOG_DBG("FHN State: rotating advertising payload");
	LOG_DBG("FHN State: next rotation in %d [s]", next_rpa_timeout);
			      ...
			      struct bt_le_ext_adv_connected_info *info)
{
	...
	LOG_DBG("FHN State: connected to %s", addr);

	fhn_conns[bt_conn_index(info->conn)] = true;
	fhn_conn_cnt++;
	__ASSERT_NO_MSG(fhn_conn_cnt <= FHN_MAX_CONN);
	...
	if (fhn_conn_cnt < FHN_MAX_CONN) {
		err = fhn_adv_start();
		if (err) {
			LOG_ERR("FHN State: failed to start advertising: %d", err);
		}
	}
}

static int fhn_adv_stop(void)
{
	int err;

	__ASSERT(fhn_adv_set != NULL,
		"FHN State: invalid state of the advertising set");

	err = bt_le_ext_adv_stop(fhn_adv_set);
	if (err) {
		LOG_ERR("bt_le_ext_adv_stop returned error: %d", err);
		return err;
	}

	LOG_DBG("FHN State: advertising stopped");
...
static int fhn_adv_start(void)
{
	...
	if (fhn_conn_cnt >= FHN_MAX_CONN) {
		LOG_DBG("FHN State: unable to resume advertising: no free connection slots");
		return -ENOMEM;
	}

	err = bt_le_ext_adv_start(fhn_adv_set, &ext_adv_start_param);
	if (err && (err != -EALREADY)) {
		LOG_ERR("bt_le_ext_adv_start returned error: %d", err);
		return err;
	}

	LOG_DBG("FHN State: advertising %s",
		(err == -EALREADY) ? "is still active" : "started");

	return 0;
}

static int fhn_tx_power_set(uint16_t handle, int8_t *tx_power)
{
	int err;
	...
	if (!tx_power) {
		LOG_ERR("FHN State: no TX power param");
		return -EINVAL;
	}

	buf = bt_hci_cmd_alloc(K_FOREVER);
	if (!buf) {
		LOG_ERR("FHN State: cannot allocate buffer to set TX power");
		return -ENOMEM;
	}
	...
	if (err) {
		LOG_ERR("FHN State: cannot set TX power (err: %d)", err);
		return err;
	}
...
static int fhn_adv_set_rotate(void)
{
	int err;
	 ...
	 * with FHN advertising using rpa_expired callback.
	 */
	err = bt_le_oob_get_local(fhn_adv_param.id, &oob);
	if (err) {
		LOG_ERR("FHN State: bt_le_oob_get_local failed: %d", err);
		return err;
	}

	LOG_DBG("FHN State: forcing advertising payload rotation");

	return 0;
}

static int fhn_adv_set_teardown(void)
{
	int err;
	...
	err = bt_le_ext_adv_delete(fhn_adv_set);
	if (err) {
		LOG_ERR("FHN State: bt_le_ext_adv_delete returned error: %d", err);
		return err;
	}
	...
	(void) k_work_cancel(&fhn_disconnected_work);

	return 0;
}

static int fhn_adv_set_setup(void)
{
	int err;
	uint8_t adv_handle;
	int8_t tx_power;
	static struct bt_le_ext_adv_cb fhn_adv_set_cb = {
		.connected = fhn_adv_connected,
		.rpa_expired = fhn_adv_rpa_expired,
	};

	__ASSERT(fhn_adv_set == NULL,
		"FHN State: invalid state of the advertising set");

	/* Create the FHN advertising set. */
	err = bt_le_ext_adv_create(&fhn_adv_param,
				   &fhn_adv_set_cb,
				   &fhn_adv_set);
	if (err) {
		LOG_ERR("FHN State: bt_le_ext_adv_create returned error: %d", err);

		/* Additional context for specific error codes. */
		if (err == -ENOMEM) {
			LOG_ERR("FHN State: bt_le_ext_adv_create returning -ENOMEM (%d) is "
			...
			LOG_ERR("FHN State: bt_le_ext_adv_create returning -EINVAL (%d) is "
				"typically caused by incorrect advertising parameters "
				"configuration that is set in the bt_fast_pair_fhn_adv_param_set "
				"API function", err);
		}
	...
	/* Set the TX power for FHN advertising and connections. */
	err = bt_hci_get_adv_handle(fhn_adv_set, &adv_handle);
	if (err) {
		LOG_ERR("FHN State: bt_hci_get_adv_handle returned error: %d", err);
		(void) fhn_adv_set_teardown();
		return err;
	}

	tx_power = FHN_TX_POWER;
	err = fhn_tx_power_set(adv_handle, &tx_power);
	if (err) {
		LOG_ERR("FHN State: fhn_tx_power_set returned error: %d", err);
		(void) fhn_adv_set_teardown();
		return err;
	}

	if (tx_power != FHN_TX_POWER) {
		LOG_WRN("FHN State: the desired TX power configuration of %d dBm"
			" is different from the actual TX power of %d dBm"
			" due to the \"%s\" board limitations",
			FHN_TX_POWER, tx_power, CONFIG_BOARD);
	} else {
		LOG_DBG("FHN State: TX power set to %d [dBm]", tx_power);
	}

	fhn_adv_set_tx_power = tx_power;
...
static void fhn_disconnected_work_handle(struct k_work *work)
{
	...
	if (err) {
		LOG_ERR("FHN State: fhn_adv_set_rotate failed: %d", err);
		return;
	}
...
static void fhn_disconnected(struct bt_conn *conn, uint8_t reason)
{
	bool max_conn = false;
	char addr[BT_ADDR_LE_STR_LEN];

	if (!fhn_conns[bt_conn_index(conn)]) {
		return;
	}
	...
	LOG_DBG("FHN State: disconnected from %s (reason %" PRIu8 ")", addr, reason);

	__ASSERT_NO_MSG(fhn_conn_cnt > 0);
	fhn_conns[bt_conn_index(conn)] = false;
	fhn_conn_cnt--;
	...
	if (!bt_fast_pair_fhn_is_provisioned()) {
		/* FHN is unprovisioned. */
		return;
	}

	if (max_conn) {
		k_work_submit(&fhn_disconnected_work);
	}
}

BT_CONN_CB_DEFINE(fhn_conn_callbacks) = {
	.disconnected = fhn_disconnected,
};

static enum dult_near_owner_state_mode fhn_utp_to_dult_near_owner_map(bool utp_active)
{
	return utp_active ? DULT_NEAR_OWNER_STATE_MODE_SEPARATED :
			    DULT_NEAR_OWNER_STATE_MODE_NEAR_OWNER;
}

static void fhn_utp_mode_state_reset(void)
{
	utp_mode = false;
	utp_mode_rpa_change_request = false;
	utp_rotations = 0;
...
static int fhn_utp_mode_state_set(bool activated, uint8_t *control_flags)
{
	int err;
	 ...
	 * to near-owner state.
	 *
	 * Guidelines specific to FHN to be compliant with DULT spec:
...
{
	return fhn_utp_mode_state_set(true, &control_flags);
}

int fp_fhn_state_utp_mode_deactivate(void)
{
	return fhn_utp_mode_state_set(false, NULL);
}
...
int fp_fhn_state_eid_read(uint8_t *eid)
{
	...
	memcpy(eid, fhn_eid, FP_FHN_STATE_EID_LEN);
...
uint8_t fp_fhn_state_ecc_type_encode(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_FAST_PAIR_FHN_ECC_SECP256R1)) {
		return ECC_TYPE_SECP256R1;
	}

	__ASSERT(0, "FHN State: incorrect ECC type selection");
...
int8_t fp_fhn_state_tx_power_encode(void)
{
	...
	calibrated_tx_power = fhn_adv_set_tx_power;
	calibrated_tx_power += FHN_TX_POWER_CORRECTION_VAL;

	__ASSERT(calibrated_tx_power >= FHN_TX_POWER_CALIBRATED_MIN,
		 "FHN State: calibrated TX power is too low");
	__ASSERT(calibrated_tx_power <= FHN_TX_POWER_CALIBRATED_MAX,
		 "FHN State: calibrated TX power is too high");
...
static void fhn_conn_uninit_iterator(struct bt_conn *conn, void *user_data)
{
	...
	/* Validate the FHN peer. */
	err = bt_conn_get_info(conn, &conn_info);
	__ASSERT_NO_MSG(!err);

	if ((conn_info.state != BT_CONN_STATE_CONNECTED) ||
	    !fhn_conns[bt_conn_index(conn)]) {
		return;
	}

	/* Disconnect the FHN peer. */
	err = bt_conn_disconnect(conn, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
	if (err) {
		LOG_ERR("FHN State: bt_conn_disconnect returned error: %d", err);
		return;
	}

	bt_addr_le_to_str(bt_conn_get_dst(conn), addr, sizeof(addr));
	LOG_DBG("FHN State: disconnecting peer: %s", addr);
}

static void fhn_conn_state_reset(void)
{
	bt_conn_foreach(BT_CONN_TYPE_LE, fhn_conn_uninit_iterator, NULL);
}

static int fhn_storage_unprovision(void)
{
	int err;
	...
	memset(fhn_eid, 0, FP_FHN_STATE_EID_LEN);

	return 0;
}

static int fhn_unprovision(void)
{
	int err;
	...
	err = fhn_adv_stop();
	if (err) {
		LOG_ERR("FHN State: fhn_adv_stop failed: %d", err);
		return err;
	}

	fhn_utp_mode_state_reset();

	err = fhn_storage_unprovision();
	if (err) {
		LOG_ERR("FHN State: fhn_storage_unprovision failed: %d", err);
		return err;
	}
...
static int fhn_reprovision(void)
{
	...
	LOG_DBG("FHN State: Successful reprovisioning of the EIK");

	return 0;
}

static int fhn_new_provision(void)
{
	int err;

	if (fhn_conn_cnt > 0) {
		/* Show a warning if there are any connections while provisioning. */
		LOG_WRN("FHN State: detected obsolete connections from previous provisioning");
	}

	/* Provision operation */
	fp_fhn_callbacks_provisioning_state_changed_notify(true);
	...
	err = fhn_adv_start();
	if (err) {
		LOG_ERR("FHN State: fhn_adv_start failed: %d", err);
		return err;
	}

	LOG_DBG("FHN State: Successful provisioning of the EIK");
...
static int fhn_provision(const uint8_t *eik)
{
	...
	if (was_provisioned) {
		return fhn_reprovision();
	} else {
		return fhn_new_provision();
	}
}
	...
	const struct bt_fast_pair_fhn_adv_param *adv_param)
{
	...
	if (fhn_adv_set) {
		...
		memcpy(&new_param, &fhn_adv_param, sizeof(new_param));
		...
		err = bt_le_ext_adv_update_param(fhn_adv_set, &new_param);
		if (err) {
			LOG_ERR("FHN State: bt_le_ext_adv_update_param returned error: %d", err);

			if (err == -EINVAL) {
				LOG_ERR("FHN State: bt_le_ext_adv_update_param returning -EINVAL "
					"(%d) is typically caused by incorrect advertising "
					"parameters configuration that is passed to the "
					"bt_fast_pair_fhn_adv_param_set API function", err);
			}

			new_param_valid = false;
		}

		if ((fhn_conn_cnt < FHN_MAX_CONN) && bt_fast_pair_fhn_is_provisioned()) {
			int adv_start_err;

			adv_start_err = fhn_adv_start();
			if (adv_start_err) {
				LOG_ERR("FHN State: cannot restore advertising state: %d",
					adv_start_err);
	...
	if (new_param_valid) {
		/* New advertising parameters accepted if the FHN advertising set is used.
		 * Otherwise they are scheduled for validation during the bt_fast_pair_enable
		 * operation (in the fp_fhn_state_init function).
		 */
		fhn_adv_param.interval_max = adv_param->interval_max;
		fhn_adv_param.interval_min = adv_param->interval_min;
	}
...
static void fhn_state_battery_level_changed(void)
{
	...
	fhn_adv_data_async_update();
}

static struct fp_fhn_battery_cb fhn_state_battery_cb = {
	.battery_level_changed = fhn_state_battery_level_changed,
};
...
int fp_fhn_state_init(void)
{
	int err;

	if (is_enabled) {
		LOG_WRN("FHN State: module already initialized");
		return 0;
	}

	/* Set the default advertising parameters if they are not already set. */
	if (fhn_adv_param.interval_min == 0) {
		struct bt_fast_pair_fhn_adv_param *adv_param_default;

		adv_param_default = BT_FAST_PAIR_FHN_ADV_PARAM_DEFAULT;
		err = bt_fast_pair_fhn_adv_param_set(adv_param_default);
		if (err) {
			LOG_ERR("FHN State: bt_fast_pair_fhn_adv_param_set failed: %d", err);
			return err;
		}
	}

	/* Subscribe to the battery level changes. */
	err = fp_fhn_battery_cb_register(&fhn_state_battery_cb);
	if (err) {
		LOG_ERR("FHN State: fp_fhn_battery_cb_register failed: %d", err);
		return err;
	}

	/* Reserve the FHN advertising set only in the Fast Pair enabled state. */
	if (!fhn_adv_set) {
		err = fhn_adv_set_setup();
		if (err) {
			LOG_ERR("FHN State: fhn_adv_set_setup failed: %d", err);
			return err;
		}
	}

	k_work_submit(&fhn_post_init_work);

	is_enabled = true;

	LOG_DBG("FHN State: enabled");

	return 0;
}

int fp_fhn_state_uninit(void)
{
	int err;

	is_enabled = false;

	/* Release the FHN advertising set only in the Fast Pair disabled state. */
	if (fhn_adv_set) {
		err = fhn_adv_stop();
		if (err) {
			LOG_ERR("FHN State: fhn_adv_stop failed: %d", err);
		...
		err = fhn_adv_set_teardown();
		if (err) {
			LOG_ERR("FHN State: fhn_adv_set_teardown failed: %d", err);
			return err;
		}
	}
